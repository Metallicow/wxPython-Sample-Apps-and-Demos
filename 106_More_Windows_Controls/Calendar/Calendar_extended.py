...
__wxPyOnlineDocs__ = 'https://wxpython.org/Phoenix/docs/html/wx.lib.calendar.html'
...
#        self.calend.ShowWeekEnd()
    ...
    def MakeToolMenu(self):
        ...
        mID = wx.NewId()
        SetToolPath(self, tb, mID, wx.Bitmap(gBmpDir + os.sep + 'DbDec.bmp'), 'Dec Year')
        self.Bind(wx.EVT_TOOL, self.OnDecYear, id=mID)

        mID = wx.NewId()
        SetToolPath(self, tb, mID, wx.Bitmap(gBmpDir + os.sep + 'Dec.bmp'), 'Dec Month')
        self.Bind(wx.EVT_TOOL, self.OnDecMonth, id=mID)

        mID = wx.NewId()
        SetToolPath(self, tb, mID, wx.Bitmap(gBmpDir + os.sep + 'Pt.bmp'), 'Current Month')
        self.Bind(wx.EVT_TOOL, self.OnCurrent, id=mID)

        mID = wx.NewId()
        SetToolPath(self, tb, mID, wx.Bitmap(gBmpDir + os.sep + 'Inc.bmp'), 'Inc Month')
        self.Bind(wx.EVT_TOOL, self.OnIncMonth, id=mID)

        mID = wx.NewId()
        SetToolPath(self, tb, mID, wx.Bitmap(gBmpDir + os.sep + 'DbInc.bmp'), 'Inc Year')
...
class PrintCalend:
    ...
    def Preview(self):
        ...
        fr = wx.Frame(None)
        frame = wx.PreviewFrame(self.preview, fr, "Print preview")

        frame.Initialize()
        frame.SetPosition(fr.GetPosition())
        frame.SetSize(fr.GetSize())
        frame.Show(True)
...
class TestApp(wx.App):
    def OnInit(self):
        gMainWin = TestFrame(None, size=(650, 400))
